...
resource "null_resource" "config" {
  ...
  provisioner "local-exec" {
    command = "echo node1 ansible_host=${aws_instance.nodes[0].public_ip} ansible_user=ec2-user > D:/EAA/DEVOPS/clarusway/myprojects/DevOps_Project/hands-on/Ansible/local-icin-tf-dosyasi"
  }
  provisioner "local-exec" {
    command = "echo node2 ansible_host=${aws_instance.nodes[1].public_ip} ansible_user=ec2-user >> D:/EAA/DEVOPS/clarusway/myprojects/DevOps_Project/hands-on/Ansible/local-icin-tf-dosyasi"
  }

  provisioner "local-exec" {
    command = "echo node3 ansible_host=${aws_instance.nodes[2].public_ip} ansible_user=ubuntu >> D:/EAA/DEVOPS/clarusway/myprojects/DevOps_Project/hands-on/Ansible/local-icin-tf-dosyasi"
  }
}
